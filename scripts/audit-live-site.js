...
(async () => {
  ...
  for (const url of urls) {
    ...
    ok(response.ok, `${url} returned ${response.status}`);
    const html = await response.text();
    ok(html.includes(`<link rel="canonical" href="${url}">`), `${url} has the wrong canonical`);
  }
  const cases = [
    ["/index.html", "/"],
    ["/services", "/services/"],
  ...
  for (const [from, to] of cases) {
    ...
    ok(response.status === 301, `${from} should return 301, received ${response.status}`);
  ...
  if (manifestResponse.ok && urls.some((url) => url.includes("/insights/"))) {
    ...
    if (firstInsight) {
      ...
      ok(response.status === 301, `Legacy Insight route returned ${response.status}`);
    }
  }
  if (CHECK_WWW) {
    const host = new URL(SITE_URL).hostname;
    const response = await request(`https://www.${host}/`, { redirect: "manual" });
    ok(response.status === 301 && location(response).startsWith(SITE_URL), "www hostname is not permanently redirected");
  }
